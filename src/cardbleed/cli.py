...
click.rich_click.OPTION_GROUPS = {"*": [
    ...
    {"name": "Synthesis",
     "options": ["--mode", "--sample", "--trim", "--jitter", "--jitter-smooth",
                 "--jitter-cross", "--shuffle", "--noise", "--smudge",
                 "--seam-feather", "--corner-guard", "--halo", "--seed"]},
    {"name": "Input / Output",
     "options": ["--out-dir", "--suffix", "--compare", "--force",
                 "--recursive", "--dry-run", "--version", "--help"]},
]}
...
@click.option("--mode", type=click.Choice(["smart", "naive"]), default="smart",
              show_default=True,
              help="[cyan]smart[/]: tone-mirrored, jittered band resampling · "
...
@click.option("--shuffle", type=float, default=48.0, show_default=True,
              metavar="PX",
