...
# squaring: background = anything that isn't the card border — transparent, or an
# extreme (near-black / near-white) scan margin — reachable from the image edge.
_SQ_ALPHA = 128.0
_SQ_DARK, _SQ_BRIGHT = 45.0, 232.0
_SQ_MAX_FRAC = 0.15  # more edge background than this = card floats in a margin, skip
...
def square_background(arr: np.ndarray, notes: list[str]) -> np.ndarray:
    ...
    Background is transparent, near-black, or near-white pixels reachable from
    the image border by a flood fill — the rounded-corner triangles or a ragged
    cut edge. Each is replaced by its nearest non-background (border) pixel, so
    the corner becomes clean square border before bleeding. Opaque artwork is
    never touched; a no-op when there's no such background, and skipped when the
    card floats in a large margin (not a corner case).
    """
    h, w = arr.shape[:2]
    c = arr.shape[2] if arr.ndim == 3 else 1
    lum = arr[..., :3].mean(axis=2) if c >= 3 else arr[..., 0].astype(np.float32)
    bg = (lum < _SQ_DARK) | (lum > _SQ_BRIGHT)
    if c in (2, 4):
        bg = bg | (arr[..., -1] < _SQ_ALPHA)
    if not bg.any():
        return arr
    ...
    while True:  # flood inward through contiguous background from the edges
        ...
        if np.array_equal(g, ext):
            break
        ext = g
    if not ext.any() or ext.mean() > _SQ_MAX_FRAC:
        return arr
